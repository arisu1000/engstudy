...
com.wcjung.engstudy/
├── data/
│   ├── local/          # Room DB, DAOs, Entities (WordEntity, EduWordEntity 등)
│   ├── repository/     # Repository 구현체 (EduWordRepository 포함)
│   └── datastore/      # DataStore UserPreferences
├── domain/
│   ├── model/          # Domain 모델 (Word, EduWord, EduLevel 등)
...
│   └── screen/         # 18개 화면 (home, study, flashcard, quiz, spelling, review, wordlist, worddetail, bookmarks, search, statistics, settings, profile, eduhome, eduwordlist, eduflashcard, eduquiz, wronganswer)
...
- DB version 5 / `fallbackToDestructiveMigration()` 적용 (개발 중 파괴적 마이그레이션 허용)
