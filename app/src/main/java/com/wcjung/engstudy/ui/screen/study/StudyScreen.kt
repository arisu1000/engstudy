...
fun StudyScreen(
    ...
    Column(
        ...
        Column(
            ...
            // 단계 선택
            Text("단계 선택", style = MaterialTheme.typography.titleSmall)
            FlowRow(
                horizontalArrangement = Arrangement.spacedBy(8.dp),
                verticalArrangement = Arrangement.spacedBy(8.dp)
            ) {
            ...
            StudyModeCard(
                ...
                onClick = { onStartFlashCard(selectedDomain?.key, selectedStage?.level) }
            )
            StudyModeCard(
                title = "퀴즈",
            ...
            StudyModeCard(
                ...
                onClick = { onStartSpellingQuiz(selectedDomain?.key, selectedStage?.level) }
            )
            StudyModeCard(
                title = "단어 목록",
                description = "선택한 분야/단계의 단어를 탐색합니다",
                icon = Icons.AutoMirrored.Filled.LibraryBooks,
                onClick = { onNavigateToWordList(selectedDomain?.key, selectedStage?.level) }
            )
